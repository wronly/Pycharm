...
detailTable = excel_table_byindex("D:\\2\\20171227\\1-1 chichang.xls")
fhTable = excel_table_byindex("D:\\2\\20171227\\1-1 liushui.xls")
'''
...
for row in fhTable:
    ...
    for row in detailTable:
        if (row['证券代码'] == stkcode) and (int(row['备份时间']) > ksr) and (int(row['备份时间']) <= int(20161231) ) :
            stkList.append(row)
...
for i in range(result.__len__()):
    ...
    wTable.save("D:\\2\\dai\\res1-1.xls")
