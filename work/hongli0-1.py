...
detailTable = excel_table_byindex("D:\\2\\20171228\\0-1 chichang.xls")
fhTable = excel_table_byindex("D:\\2\\20171228\\0-1 liushui.xls")
'''
...
for row in fhTable:
    ...
    for row in detailTable:
        if (row['证券代码'] == stkcode) and (int(row['备份时间']) > ksr) and (int(row['备份时间']) <= int(20170101) ) :
            stkList.append(row)
    ...
    if stkList.__len__()==0:
        ...
        fhDetail['最早持有日期'] = '>=20170101'
        fhDetail['登记股份']=0
        fhDetail['免税股数']=0
        fhDetail['免税金额']=0
        fhDetail['退税金额']=0
        fhDetail['红利金额']=fhDetail['红利金额']
        fhDetail['登记日']  =djr
        result.append(fhDetail)
        continue
    '''
...
for i in range(result.__len__()):
    ...
    sheet1.write(i+1, 7, float(result[i]['红利金额']))
